...
if command -v curl &> /dev/null; then
    if curl -s "http://127.0.0.1:7700/health" > /dev/null 2>&1; then
        ...
        if [ "$INDEX_EXISTS" = "yes" ]; then
            ...
            printf "   ${YELLOW}uv run python scripts/step3_migrate_data.py${NC}\n"
        fi
    else
...
echo "   uv run python scripts/step5_test.py"
echo ""
printf "   ${YELLOW}# 更新索引（全量重建）${NC}\n"
echo "   uv run python scripts/step3_migrate_data.py"
echo ""
printf "   ${YELLOW}# 更新索引（增量，最近 1 小时）${NC}\n"
echo "   uv run python scripts/incremental_update.py --recent 60"
echo ""
